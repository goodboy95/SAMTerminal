# 注册邮件验证（ALTCHA + 邮件验证码）文档索引

本目录文档用于实现“注册时邮件验证”功能，覆盖：
- 前台（用户注册）流程：ALTCHA 校验后发送验证码邮件，输入验证码后允许注册。
- 后台（/admin）“邮件验证管理”页面：SMTP 配置、发送日志、IP 统计与封禁。

## 建议阅读顺序
1. `doc/captcha/development.md`：总体方案与端到端开发步骤（前台、后台、后端、DB）。
2. `doc/captcha/altcha.md`：ALTCHA 在本项目的接入方式（含建议的 Sentinel 部署与接口形态）。
...
- `doc/captcha/altcha.md`：ALTCHA（PoW）接入指南（前端 widget、后端 challenge/verify、与验证码发送接口的组合方式）。
...
- “额外限流阈值”：对 challenge/verify/send/verify-code 四类接口给出建议阈值与实现建议。
